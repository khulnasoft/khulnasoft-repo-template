...
# PURPOSE:       Shell script that setups the @khulnasoft/khulnasoft-repo-template GitHub project template.
...
echo -e "Thanks for using ${GREEN}@khulnasoft/khulnasoft-repo-template${NC}"
echo -e "Read all the documentation carefully before you continue executing this script: ${UPURPLE}https://github.com/khulnasoft/khulnasoft-repo-template${NC}\n"
...
y | Y)
  ...
  find .github/ -type f -name "*" -print0 | xargs -0 sed -i "s/khulnasoft/${NEW_USERNAME}/g"
  find .github/ -type f -name "*" -print0 | xargs -0 sed -i "s/info@khulnasoft.com/${NEW_EMAIL}/g"
  find .github/ -type f -name "*" -print0 | xargs -0 sed -i "s/khulnasoft-repo-template/${PROJECT_NAME}/g"
  find .gitignore -type f -name "*" -print0 | xargs -0 sed -i "s/khulnasoft\/khulnasoft-repo-template/${NEW_USERNAME}\/${PROJECT_NAME}/g"
  ...
  if [ $will_omit_commit = false ]; then                                                  # if the ignore option for tests has been specified
    ...
    git commit -m "📝 Set up '@khulnasoft/khulnasoft-repo-template' template: Personalized files by executing the SETUP_TEMPLATE.sh script.🚀"
